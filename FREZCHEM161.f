C  FREZCHEM16: This is a chemical thermodynamic model for aqueous
...
C  and nitrogen chemistries.  This version contains a 
...
C  Model will calculate the heat capacity of seawater.The model parameter
C  numbers were changed to make room for new cations and anions.  
     ...
     X,CJPHI(300,300)
      COMMON /B/ X(300),AX(300),K(300),KV(300),K0(300),AH2O,PHI,
     XITER,NCAR,MIX,IRON,GX(300),DEL(5),C0(15,16:30),       
     XIX(300),TEMP,I,FX(300),ACID,C1(15,16:30),XY(300)
     X,PB,GXV(300),V0(300),WT(300),RHO,VH2O,YESORNO,XN26H2O,
     XCO2MF,AX29I,TCAR,ZZZ,TMET,CH4MF,XCO26H2O,XCH46H2O,MIX3,
     XAB1(15,16:30),AB2(15,16:30),PSII1,PH,SA,S,CARBSS,MIX2
      COMMON /T/TITLE
...
C	IF((AX(209).GT.0.0).AND.(AX(214).GT.0.0))THEN
...
C	IF((AX(239).GT.0.0).AND.(AX(214).GT.0.0))THEN
...
C NCAR OPTION REMOVED FROM FC16	
	...
	IF(AX(209).GT.0.0)THEN
		XX = AX(209)
		CALL GAS(209,XX,TEMP,GX(209),ZZZ)
		X(209)=0.0
	...
	IF(AX(214).GT.0.0)THEN
		XX = AX(214)
		CALL GAS(214,XX,TEMP,GX(214),ZZZ)
		X(214)=0.0
	...
	IF(AX(239).GT.0.0)THEN
		XX = AX(239)
		CALL GASN2(239,XX,TEMP,GX(239),ZZZ)
		X(239)=0.0
...
183	IF(AX(211).GT.0.0)X(212)=K(212)*AX(211)/GX(212)
	...
	IF((EQ1AH2O.LE.EQ2AH2O).AND.(EQ1AH2O.LT.K(31))
     X.AND.(EQ1AH2O.LT.EQ3AH2O))CASE = 1
	IF((EQ2AH2O.LE.EQ1AH2O).AND.(EQ2AH2O.LT.K(31))
     X.AND.(EQ2AH2O.LT.EQ3AH2O))CASE = 2
	IF((K(31).LT.EQ2AH2O).AND.(K(31).LT.EQ1AH2O)
     X.AND.(K(31).LT.EQ3AH2O))CASE = 3
     	IF((EQ3AH2O.LT.EQ1AH2O).AND.(EQ3AH2O.LT.K(31))
     X.AND.(EQ3AH2O.LT.EQ2AH2O))CASE = 4
...
C MOLLER (T>325K) K(49) = EXP(-1.193844E1+3.728778E-2*T-1.025801E-4*T**2)
...
C  of water using the Pitzer equations.
	...
	COMMON /A/ AKI(0:20),AKII(0:20),MAXCAT,MAXAN,BET0(300,300),
     ...
     X,CJPHI(300,300)
	COMMON /B/ X(300),AX(300),K(300),KV(300),K0(300),AH2O,PHI,
     XITER,NCAR,MIX,IRON,GX(300),DEL(5),C0(15,16:30),K146       
     XIX(300),TEMP,I,FX(300),ACID,C1(15,16:30),XY(300)
     X,PB,GXV(300),V0(300),WT(300),RHO,VH2O,YESORNO,XN26H2O,
     XCO2MF,AX29I,TCAR,ZZZ,TMET,CH4MF,XCO26H2O,XCH46H2O,MIX3,
     XAB1(15,16:30),AB2(15,16:30),PSII1,PH,SA,S,CARBSS,MIX2
...
C FOR METHANE AND NITROGEN HYDRATE
      ...
      ANS = 1.231401E2 -8.466764E-1*T +1.462864E-3*T**2
...
C  Subroutine prints the results.
	...
	REAL MOLE, IX, I, LAM
	INTEGER OPT
	CHARACTER*20 SPECIS(70), SOLID(31:147), SPECIES(300)
	CHARACTER*50 TITLE
	DIMENSION SUMI(210)
...
821	IF (AX(209).GT.0.0) THEN
	...
	GX(232) = GX(3)
	AX(232) = X(232)*GX(232)
	WRITE(2,805)SPECIS(62),IX(232),X(232),GX(232),AX(232),FX(232)
        END IF
...
1595	BALAN = X(1) + X(2) + 2*X(3) + 2*X(4) - X(16) -2*X(17)+X(6)
	...
	CALL BORON
	CALL FLUORIDE
C	IF (X(232).GT.0.0)CALL SRCO3IP
...
132			CONTINUE
		...
		IF((J.EQ.235).OR.(J.EQ.237))GO TO 135
				X(J)=X(J)/DEL(KA)
135			CONTINUE
...
152			CONTINUE
		...
		IF((J.EQ.235).OR.(J.EQ.237))GO TO 155
				X(J)=X(J)*DEL(KA)
155			CONTINUE
...
C  Subroutine for FECL3¥2KCL¥H2O equilibrium.
...
C  Subroutine for FE2(SO4)3¥2K2SO4¥14H2O equilibrium.
...
C  Subroutine for K2SO4¥FESO4¥6H2O equilibrium.
...
C  Subroutine for NA2SO4¥FESO4¥4H2O equilibrium.
...
C  Subroutine for FE2(SO4)2¥H2SO4¥8H2O equilibrium.
...
C  Subroutine for KAL(SO4)2¥12H2O equilibrium.
...
C  Subroutine for NAAL(SO4)2¥12H2O equilibrium.
...
C  Subroutine for FESO4¥AL2(SO4)3¥22H2O equilibrium.
...
C  Subroutine for MGSO4¥AL2(SO4)3¥22H2O equilibrium.
...
920	FORMAT('PHI=',F10.6,5X,'SUMGAMMA=',F10.6,5X,'PSIS=',F10.6)
	WRITE(2,930) S, SA
930	FORMAT("PRACTICAL SALINITYÅ",F8.4,5X,"ABSOLUTE SALINITY=",F8.4)
...
C  Subroutine for NH3.H2O and NH3.2H2O equilibrium.
	...
	DOUBLE PRECISION AH2O,PHI,KPRIME1,KPRIME2
